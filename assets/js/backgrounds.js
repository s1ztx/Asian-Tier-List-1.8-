...
(function(){
  ...
  function mount(effects){
    ...
    // subtle mouse-parallax on the whole layer for depth — cheap, high impact
    ...
    function parallaxLoop(){
      ...
      host.style.transform = `translate(${px}px, ${py}px) scale(1.03)`;
      requestAnimationFrame(parallaxLoop);
    }
    if(!REDUCE) parallaxLoop();
  ...
  function auroraLayer(){
    const bands = Array.from({length:4},(_,i)=>({
      hueA: ['#e63946','#3a86ff','#2ec4b6','#ffd60a'][i],
      phase: rand(0,1000), amp: rand(70,130), speed: rand(.002,.004), yBase: .12+i*.15
    }));
    return { draw(ctx,w,h,t){
      ctx.save(); ctx.globalCompositeOperation='screen';
      bands.forEach(b=>{
        const grad = ctx.createLinearGradient(0,0,w,0);
        grad.addColorStop(0,'transparent'); grad.addColorStop(.5,b.hueA+'55'); grad.addColorStop(1,'transparent');
        ctx.fillStyle = grad;
        ctx.beginPath();
  ...
  function hexGridLayer(){
    ...
    return { draw(ctx,w,h,t){
      ctx.save();
      ctx.strokeStyle='rgba(140,170,220,.13)'; ctx.lineWidth=1;
      const hw=size, hh=size*.87;
      const drift = (t*.08)%(hw*3);
  ...
  function particlesLayer(n=120, palette=['#e63946','#3a86ff','#ffd60a']){
    let pts=[];
    return {
      resize(w,h){ pts = Array.from({length:n},()=>({ x:rand(0,w), y:rand(0,h), r:rand(.7,2.6), vy:rand(.08,.32), vx:rand(-.1,.1), c:palette[Math.floor(Math.random()*palette.length)], a:rand(.3,.95) })); },
      draw(ctx,w,h){
        pts.forEach(p=>{
          p.y-=p.vy; p.x+=p.vx;
  ...
  function beamsLayer(){
    const beams = Array.from({length:6},(_,i)=>({ x: rand(0,1), w: rand(70,180), speed: rand(.0015,.003), phase: i*40 }));
    return { draw(ctx,w,h,t){
      ctx.save(); ctx.globalCompositeOperation='screen';
      beams.forEach(b=>{
        const bx = w*b.x + Math.sin(t*b.speed+b.phase)*w*.15;
        const grad = ctx.createLinearGradient(bx-b.w,0,bx+b.w,0);
        grad.addColorStop(0,'transparent'); grad.addColorStop(.5,'rgba(255,255,255,.07)'); grad.addColorStop(1,'transparent');
  ...
  /* ---------------- shared: shooting stars, usable as an accent layer ---------------- */
  function shootingStarsLayer(){
    let stars=[];
    return {
      resize(w,h){ stars = Array.from({length:4},()=>({ active:false, x:0,y:0,vx:0,vy:0,life:0,wait:rand(60,300) })); },
      draw(ctx,w,h){
        stars.forEach(s=>{
          if(!s.active){
            s.wait--;
            if(s.wait<=0){ s.active=true; s.x=rand(0,w*.6); s.y=rand(0,h*.3); s.vx=rand(6,11); s.vy=rand(3,6); s.life=0; }
            return;
          }
          s.x+=s.vx; s.y+=s.vy; s.life++;
          const a = Math.max(0, 1-s.life/40);
          ctx.strokeStyle=`rgba(255,255,255,${a*.8})`; ctx.lineWidth=1.6;
          ctx.beginPath(); ctx.moveTo(s.x,s.y); ctx.lineTo(s.x-s.vx*6,s.y-s.vy*6); ctx.stroke();
          if(s.life>40 || s.x>w || s.y>h){ s.active=false; s.wait=rand(120,400); }
        });
      }
    };
  }
      ...
      draw(ctx,w,h){
        ...
        cols.forEach(c=>{
          ...
          for(let i=0;i<c.len;i++){
            const a = 1-i/c.len;
            ctx.fillStyle=`rgba(46,196,182,${a*.7})`;
  ...
  function starsLayer(){
    let stars=[];
    return {
      resize(w,h){ stars = Array.from({length:140},()=>({ x:rand(0,w), y:rand(0,h), r:rand(.5,2), tw:rand(0,10) })); },
      draw(ctx,w,h,t){
        stars.forEach(s=>{
          const a = .4+Math.sin(t*.02+s.tw)*.4;
  ...
  /* ---------------- SEARCH: radar sweep + drifting locators ---------------- */
  function radarSweepLayer(){
    return { draw(ctx,w,h,t){
      const cx=w*.5, cy=h*.42;
      ctx.save();
      for(let r=60;r<Math.max(w,h)*.6;r+=90){
        ctx.strokeStyle='rgba(58,134,255,.08)'; ctx.lineWidth=1;
        ctx.beginPath(); ctx.arc(cx,cy,r,0,Math.PI*2); ctx.stroke();
      }
      const angle = t*.012;
      const grad = ctx.createConicGradient ? ctx.createConicGradient(angle,cx,cy) : null;
      if(grad){
        grad.addColorStop(0,'rgba(58,134,255,.22)');
        grad.addColorStop(.06,'rgba(58,134,255,0)');
        grad.addColorStop(1,'rgba(58,134,255,0)');
        ctx.fillStyle=grad;
        ctx.beginPath(); ctx.arc(cx,cy,Math.max(w,h)*.6,0,Math.PI*2); ctx.fill();
      } else {
        ctx.strokeStyle='rgba(58,134,255,.3)';
        ctx.beginPath(); ctx.moveTo(cx,cy); ctx.lineTo(cx+Math.cos(angle)*w, cy+Math.sin(angle)*w); ctx.stroke();
      }
      ctx.restore();
    }};
  }
  function locatorPingsLayer(){
    let pings=[];
    return {
      resize(w,h){ pings = Array.from({length:8},()=>({ x:rand(0,w), y:rand(0,h), life:rand(0,160) })); },
      draw(ctx,w,h){
        pings.forEach(p=>{
          p.life+=1; if(p.life>160) p.life=0;
          const r = (p.life/160)*40;
          const a = Math.max(0, .3*(1-p.life/160));
          ctx.strokeStyle=`rgba(255,214,10,${a})`; ctx.lineWidth=1.4;
  ...
  /* ---------------- COMPARE: blue vs red split energy ---------------- */
  function splitEnergyLayer(){
    return { draw(ctx,w,h,t){
      const blueGrad = ctx.createLinearGradient(0,0,w*.55,0);
      blueGrad.addColorStop(0,'rgba(58,134,255,.16)'); blueGrad.addColorStop(1,'transparent');
      ctx.fillStyle=blueGrad; ctx.fillRect(0,0,w*.55,h);
      const redGrad = ctx.createLinearGradient(w,0,w*.45,0);
      redGrad.addColorStop(0,'rgba(230,57,70,.16)'); redGrad.addColorStop(1,'transparent');
      ctx.fillStyle=redGrad; ctx.fillRect(w*.45,0,w*.55,h);
      const seamX = w*.5 + Math.sin(t*.008)*10;
      ctx.strokeStyle='rgba(255,255,255,.1)'; ctx.lineWidth=1.5;
      ctx.beginPath(); ctx.moveTo(seamX,0); ctx.lineTo(seamX,h); ctx.stroke();
    }};
  }
  function vsSparksLayer(){
    let sparks=[];
    return {
      resize(w,h){ sparks = Array.from({length:20},()=>({ x:w*.5+rand(-40,40), y:rand(0,h), side: Math.random()>.5?1:-1, vy:rand(.3,.9), life:rand(0,100) })); },
      draw(ctx,w,h){
        sparks.forEach(s=>{
          s.y-=s.vy; s.life+=1;
          if(s.y<-10||s.life>240){ s.y=h+10; s.x=w*.5+rand(-40,40); s.life=0; }
          const color = s.side>0 ? '230,57,70' : '58,134,255';
          ctx.fillStyle=`rgba(${color},.5)`;
          ctx.beginPath(); ctx.arc(s.x + s.side*Math.sin(s.life*.1)*8, s.y, 1.6, 0, 7); ctx.fill();
        });
      }
    };
  }

  /* ---------------- STATS: animated data grid + graph lines ---------------- */
  function dataGridLayer(){
    return { draw(ctx,w,h,t){
      ctx.save();
      ctx.strokeStyle='rgba(46,196,182,.08)'; ctx.lineWidth=1;
      const step=48, offset=(t*.15)%step;
      for(let x=-step+offset;x<w;x+=step){ ctx.beginPath(); ctx.moveTo(x,0); ctx.lineTo(x,h); ctx.stroke(); }
      for(let y=-step+offset;y<h;y+=step){ ctx.beginPath(); ctx.moveTo(0,y); ctx.lineTo(w,y); ctx.stroke(); }
      ctx.restore();
    }};
  }
  function graphLinesLayer(){
    const series = Array.from({length:3},(_,i)=>({
      color:['46,196,182','58,134,255','255,214,10'][i], phase:rand(0,10), amp:rand(30,60), yBase:.3+i*.2
    }));
    return { draw(ctx,w,h,t){
      series.forEach(s=>{
        ctx.strokeStyle=`rgba(${s.color},.22)`; ctx.lineWidth=1.6;
        ctx.beginPath();
        for(let x=0;x<=w;x+=16){
          const y = h*s.yBase + Math.sin(x*.02 + t*.02 + s.phase)*s.amp*.4 + Math.sin(x*.007+s.phase)*s.amp*.6;
  ...
  const REGISTRY = {
    home: ()=> [auroraLayer, hexGridLayer, ()=>particlesLayer(130), beamsLayer, shootingStarsLayer],
    ...
    reviews: ()=> [softGlowLayer, starsLayer, shootingStarsLayer],
    profile: ()=> [energyRingsLayer, particleTrailsLayer],
    search: ()=> [radarSweepLayer, locatorPingsLayer],
    compare: ()=> [splitEnergyLayer, vsSparksLayer],
    stats: ()=> [dataGridLayer, graphLinesLayer],
