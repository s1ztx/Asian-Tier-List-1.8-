...
window.ATL_initShell = function(activePage){
  ...
  <div class="container">
    ...
    <div class="footer-bottom">
      ...
      <span class="text-sm" style="display:flex; align-items:center; gap:14px;">
        <button class="btn btn-ghost btn-sm" id="devPreviewToggle" style="text-transform:none; font-family:var(--f-mono); font-size:11px;">⚙ Preview role (design QA)</button>
      </span>
    </div>
  </div>`;
  ...
  modal.id = 'devPreviewModal';
  const roles = window.ATL_DATA.DISCORD_ROLE_HIERARCHY;
  modal.innerHTML = `
  <div class="glass modal">
    <div class="modal-head">
      <div>
        <div class="eyebrow">Frontend preview tool</div>
        <h3 style="margin:6px 0 0;">Simulate a Discord role</h3>
      </div>
      <button class="modal-close" id="devModalClose">${closeIconSvg()}</button>
    </div>
    <p class="text-sm text-muted">This switches the local UI state only, so role-gated pages and panels can be reviewed before real Discord OAuth2 and role sync are connected on the backend. It does not create a real account or session.</p>
    <div class="grid-2 mt-16">
      ${roles.map(r=>`<button class="btn btn-ghost btn-sm" data-role="${r}" style="text-transform:none;">${r}</button>`).join('')}
    </div>
  </div>`;
  document.body.appendChild(modal);
  document.getElementById('devPreviewToggle').addEventListener('click', ()=> modal.classList.add('open'));
  document.getElementById('devModalClose').addEventListener('click', ()=> modal.classList.remove('open'));
  modal.addEventListener('click', (e)=>{ if(e.target===modal) modal.classList.remove('open'); });
  modal.querySelectorAll('[data-role]').forEach(btn=> btn.addEventListener('click', ()=> window.ATL_SESSION.setPreviewRole(btn.dataset.role)));
